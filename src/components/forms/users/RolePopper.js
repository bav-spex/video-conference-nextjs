  ...
  return (
    <Popper
      ...
      sx={{
        borderRadius: '5px',
        zIndex: 1200,
        width: roleAnchorEl ? roleAnchorEl.clientWidth + 4 : 'auto' // 👈 dynamic width
      }}
    >
      <ClickAwayListener onClickAway={() => setOpenRolePopper(false)}>
        <Paper>
          <Box sx={{ borderBottom: '1px solid', borderColor: theme.palette.company.lightgrey, padding: '10px' }}>
            <Typography variant='body1Bold'>Select Role</Typography>
          </Box>
          {role_dropdown.length > 0 ? (
            role_dropdown.map((item, index) => {
              return (
                <Box
                  key={item.roleId}
                  sx={{
                ...
                >
                  <Typography
                    variant='body1'
                    sx={{
                      color: roleMappingName.includes(item.role)
                  ...
                  <Box
                    sx={{
                      ...
                      alignItems: 'center',
                      background: roleMappingName.includes(item.role)
                        ? theme.palette.company.primary
                  ...
                  >
                    {roleMappingName.includes(item.role) && (
                      <CheckIcon
                        sx={{
                          width: '16px',
                          height: '16px',
