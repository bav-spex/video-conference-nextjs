...
export const getVendorPreQuestions = async (successCallback, setLoading) => {
  await apiHelper(`${authConfig.complianceDevRakshitah_base_url}vendor/pre-questions/basic`, 'get', null, {})
    .then(res => {
      successCallback(res.data)
      if (setLoading) {
        setLoading(false)
...
export const getVendorAsessmentSummary = async (vendorId, successCallback, setLoading) => {
  await apiHelper(`${authConfig.complianceDevRakshitah_base_url}vendorAssessment/vendor/assessment-summary/${vendorId}`, 'get', null, {})
    .then(res => {
      successCallback(res.data)
      if (setLoading) {
        setLoading(false)
